...
void PrivacyDialog::on_pushButtonZObsControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZObsControlDialog* zObsControl = new ZObsControlDialog(this);
    zObsControl->setModel(walletModel);
    zObsControl->exec();
}

void PrivacyDialog::setZObsControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzObsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOBS()
{
    ...
    if (!ZObsControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZObsControlDialog::GetSelectedMints();
    ...
    ZObsControlDialog::setSelectedMints.clear();
    ui->labelzObsSelected_int->setText(QString("0"));
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Obs, ";
