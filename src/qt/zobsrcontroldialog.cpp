...
std::set<std::string> ZObsControlDialog::setSelectedMints;
std::set<CMintMeta> ZObsControlDialog::setMints;

ZObsControlDialog::ZObsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZObsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZObsControlDialog::~ZObsControlDialog()
{
    delete ui;
}

void ZObsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZObsControlDialog::updateList()
{
...
void ZObsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZObsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZObs_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZObsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZObsControlDialog::GetSelectedMints()
{
...
void ZObsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
