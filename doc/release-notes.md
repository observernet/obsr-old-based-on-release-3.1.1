OBSR Core version *3.1.1* is now available from:  <https://github.com/observernet/obsr/releases>
...
OBSR Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
OBSR Core is extensively tested on multiple operating systems using
...
OBSR Core should also work on most other Unix-like systems but is not
...
### Fix spending for v1 zOBSR created before block 1050020

The transition to v2 zOBSR and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zOBSR to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zOBSR v1 will now be able to convert that into OBSR and then zOBSR v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zOBSR stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by OBSR stakers.
...
### Show progress percent for zobsr reindex operations
...
OBSR Daemon & Client (RPC Changes)
--------------
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zOBSR code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zOBSR elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zobsr reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zOBSR created before block 1050020. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/obsr-project-translations/).
